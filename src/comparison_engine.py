...
class ComparisonResult:
    ...
    cheapest_tariff: Tariff | None
    potential_savings: float  # in pence

    @property
    def should_switch(self) -> bool:
        logger.debug(
            f"cheapest_tariff: {self.cheapest_tariff.display_name}, potential_savings: {self.potential_savings}, SWITCH_THRESHOLD: {config.SWITCH_THRESHOLD}"
        )
        return (
            self.cheapest_tariff is not None
            and self.cheapest_tariff != self.current_tariff_comparison.tariff
...
class ComparisonEngine:
    ...
    def compare_tariffs(
        self, account_info: AccountInfo, available_tariffs: list[Tariff]
    ) -> ComparisonResult:

        curr_costs = self._calculate_current_cost(account_info)
        curr_comparison = TariffComparison(
            tariff=account_info.current_tariff, cost_breakdown=curr_costs
        )
        ...
        cheapest_tariff, potential_savings = self._find_best_option(
            curr_comparison, alternative_comparisons
        )
    ...
    def _find_best_option(
        self, current: TariffComparison, alternatives: list[TariffComparison]
    ) -> tuple[Tariff | None, float]:
        ...
        valid_comparisons.extend(
            [comp for comp in alternatives if comp.is_valid and comp.tariff.switchable]
        )
    ...
    def _compare_tariff(
        ...
        period_costs = self._calculate_potential_costs(
            account_info.consumption, unit_rates
        )
    ...
    def _calculate_current_cost(self, account_info: AccountInfo) -> CostBreakdown:
        total_con_cost = sum(
            float(entry["costDeltaWithTax"] or 0) for entry in account_info.consumption
        )

        # Total consumption
        total_wh = sum(
            float(consumption["consumptionDelta"])
    ...
    def _calculate_potential_costs(
        self, consumption_data: list[dict], rate_data: list[dict]
    ) -> list[dict]:
        ...
        for consumption in consumption_data:
            ...
            matching_rate = next(
                ...
                if rate["valid_from"]
                <= read_time
                <= (rate.get("valid_to") or "9999-12-31T23:59:59Z")
            ...
            cost = float(
                "{:.4f}".format(consumption_kwh * matching_rate["value_inc_vat"])
            )
    ...
    def _get_potential_tariff_rates(
        self, tariff: Tariff, region_code: str
    ) -> tuple[float, list[dict], str]:
        """
        Get rates for a specific tariff and region
        """

        all_products = self.query_service.execute_rest_query(
            f"{config.BASE_URL}/products/?brand=OCTOPUS_ENERGY&is_business=false"
        )
        product = next(
            (
                product
        ...
        filtered_region = tariff_details.get(
        ...
        region_tariffs = filtered_region.get(
            "direct_debit_monthly"
        ) or filtered_region.get("varying")
        standing_charge_inc_vat = region_tariffs.get("standing_charge_inc_vat")

        if standing_charge_inc_vat is None:
            raise ValueError(
                f"Standing charge including VAT not found for region {region_code_key}."
            )
        ...
        if not unit_rates_link:
            raise ValueError(
                f"Standard unit rates link not found for region: {region_code_key}"
            )
