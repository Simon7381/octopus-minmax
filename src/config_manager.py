...
def update_config(new_values):
    ...
    with _config_lock:
        ...
        if "one_off_run" in new_values:
            config.ONE_OFF_RUN = str(new_values["one_off_run"]).lower() in [
                "true",
                "1",
                "yes",
                "on",
        ...
        if "dry_run" in new_values:
            config.DRY_RUN = str(new_values["dry_run"]).lower() in [
                "true",
                "1",
                "yes",
                "on",
        ...
        if "batch_notifications" in new_values:
            config.BATCH_NOTIFICATIONS = str(
                new_values["batch_notifications"]
...
def validate_config(config_dict):
    ...
    if "execution_time" in config_dict:
        if not re.match(
            r"^([0-1][0-9]|2[0-3]):[0-5][0-9]$", config_dict["execution_time"]
        ):
